...
def test_parse_plan_file_basic():
  ...
  actions, states = parser.parse_file(plan_content, reference_date)
  ...
  a_light = next((a for a in actions if a.action_name.startswith('turn_on_light')), None)
  assert a_light is not None
  assert a_light.execution_time > reference_date
  # states should be an int if present in the plan file
  assert states is None or isinstance(states, int)
